...
class SensorStream(DataStream):
    ...
    def process_batch(self, data_batch: List[Any]) -> str:
        try:
            ...
            temps = [float(item.split(":")[1]) for item in data_batch if isinstance(item, str) and item.startswith("temp")]
    ...
    def filter_data(self, data_batch: List[Any], criteria: Optional[str] = None) -> List[Any]:
        try:
            if criteria == "critical":
                return [item for item in data_batch if isinstance(item, str) and item.startswith("temp") and float(item.split(":")[1]) > 30.0]
...
class TransactionStream(DataStream):
    ...
    def process_batch(self, data_batch: List[Any]) -> str:
        try:
            ...
            flows = [float(item.split(":")[1]) if item.startswith("buy") else -float(item.split(":")[1]) 
                     for item in data_batch if isinstance(item, str) and (item.startswith("buy") or item.startswith("sell"))]
            
            net = sum(flows)
            sign = "+" if net > 0 else ""
            
            return f"Transaction analysis: {len(data_batch)} operations, net flow: {sign}{int(net)} units"
        except Exception as e:
    ...
    def filter_data(self, data_batch: List[Any], criteria: Optional[str] = None) -> List[Any]:
        try:
            if criteria == "large":
                return [item for item in data_batch if isinstance(item, str) and float(item.split(":")[1]) > 1000.0]
            return super().filter_data(data_batch, criteria)
        except Exception:
            return []
...
class EventStream(DataStream):
    ...
    def process_batch(self, data_batch: List[Any]) -> str:
        try:
            ...
            errors = len([item for item in data_batch if isinstance(item, str) and item == "error"])
...
class StreamProcessor:
    ...
    def run_pipeline(self, batches: Dict[str, List[Any]]) -> None:
        ...
        print(f"Filtered results: {sensor_alerts} critical sensor alerts, {large_tx} large transaction\n")
